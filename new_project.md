...
- 本任务为全局基础规则，不要求 feature 分支，但建议在独立分支完成再合并
- 确认输出目录 `./{workspace}/` 可写，不存在则创建
...
2) 宪法文档生成  
   - 生成 `constitution.md`，包含：
     ...
     - 接口契约（OpenAPI/GraphQL/proto/事件等真源约定）
     - 安全、合规、隐私要求（如输入中有）
     - 构建与发布流程（含 CI/CD 基线）
...
4) 输出与落盘  
   - 宪法文档：写入 `./{workspace}/mydoc/{date}-{taskname}/project-constitution.md`
   - 项目级规则文件（若生成/更新）：写入仓库内约定位置（需在宪法中记录路径与用途）
   - 若需 catalog 更新，请在 `./{workspace}/mydoc/catalog.md` 补充索引（如存在）
...
- `project-constitution.md` 已生成并落盘至指定路径
